...
@appointments_router.get(path='/available/{doctor_id}')
async def get_appointment_available(
        doctor_id: int,
        appointment_service: AppointmentsService = Depends(get_appointments_service)
):
    """Получение доступных ячеек для записи"""
    available_cells = await appointment_service.get_available(doctor_id=doctor_id)
